...
If you want to include references to Kin in your app, please make use of our [Branding Materials](/docs/essentials/branding/). We have logos, icons and a style guide for how to appropriately implement Kin branding.

## Creating Accounts Efficiently

Account creation on the Solana blockchain comes with a SOL cost. Accounts with zero balances or activity are still charged a [rent fee](https://docs.solana.com/implemented-proposals/rent) by Solana. That means it is in your best interest to make sure that account creation is done efficiently, so as to avoid unnecessary fees.

Account creation is almost instantaneous and can be done on a per need basis when the user interacts with Kin for the first time.
...
However, due to changes in the Solana infrastructure that have occurred since the publishing of the latest Kin SDKs, we have found that some exchanges have deposit addresses which are incompatible.
...
A list of compatible wallets can be found [here](https://www.kin.org/ecosystem-apps).
