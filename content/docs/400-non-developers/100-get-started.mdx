...
- Go to your 'When View Mnemonic' worflow and add an action via `Element Actions / Set state`. Set the element to `Button Show Mnemonic`, the custom state to `Show Mnemonic` and the value to `no`.
...
- In 'Workflows', select 'When Button Set Up Keypair is clicked' and click on 'Step 1'. Add the value of 'mnemonic' as a dynamic value set to `Input Mnemonic's value`.
...
Now for the real stuff! We're going to send some of our User's Kin to another Public Key.
